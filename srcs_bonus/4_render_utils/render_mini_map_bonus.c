...
/*   Updated: 2024/05/28 18:00:52 by sangshin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const t_img *img = game->minimap_img;
	const char	**map = (const char **)game->map;
	...
	while (i <= py + MINI_MAP_RADIUS)
	{
		...
		{
			if (i < 0 || i > game->map_len_h || j < 0 || j > game->map_len_w)
				draw_square_on_img(img, x, y, 0x00000000);
			else if (map[i][j] == '1')
				draw_square_on_img(img, x, y, 0x00FFFFFF);
			else if (map[i][j] == 'D')
				draw_square_on_img(img, x, y, 0x00D0D7FE);
			else
				draw_square_on_img(img, x, y, 0x00000000);
			j++;
			y += MINI_MAP_PIXEL;
