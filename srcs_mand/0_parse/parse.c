...
/*   Updated: 2024/06/19 12:21:06 by sangshin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief
 * check if line is wall texture path or color
 * @param parse
 * @param g_ret
 * @param type
 * @param line
 * @return int
 */
static int	do_parsing(t_parse *parse, int type, char *line)
{
...
int	parse_file(t_game *game, const char *cub_file_path)
{
	...
	while (p.line && p.parse_type != T_MAP)
	{
		...
		else if (do_parsing(&game->parse, p.parse_type, p.line)
			== FAILURE)
